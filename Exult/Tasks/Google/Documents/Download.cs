...
namespace Exult.Tasks.Google.Documents
{
    ...
    /// </summary>
    public class Download : DocumentsTask
    {
        ...
        private void DoFileDownload(Request documentsRequest, ITaskItem document, string targetFile)
        {
            using (Stream downloadStream = documentsRequest.Download(document, ExportFormat))
            using (FileStream fileSaveStream = new FileStream(targetFile, FileMode.OpenOrCreate))
            {
                downloadStream.CopyTo(fileSaveStream);
            }
        }
        ...
        private void DoDocumentHtmlDownload(Request documentsRequest, ITaskItem document, string targetFile)
        {
            using (Stream downloadStream = documentsRequest.Download(document, Document.DownloadType.html))
            using (FileStream fileSaveStream = new FileStream(targetFile, FileMode.OpenOrCreate))
            using (StreamWriter streamWriter = new StreamWriter(fileSaveStream))
            using (StreamReader streamReader = new StreamReader(downloadStream))
            {
                ...
                while (!streamReader.EndOfStream)
                {
                    ...
                    if (line.Contains('&'))
                    {
                        foreach (var pair in _HtmlEntityReplacements)
                        {
                            line = line.Replace(pair.Key, string.Format("<xm:ent code=\"{0}\"/>", pair.Value));
                        }
                    }
        ...
        static IDictionary<string, int> _HtmlEntityReplacements = new Dictionary<string, int> {
   {"&#39;", 39},
  {"&nbsp;"   ,160}, // no-break space = non-breaking space,  U+00A0 ISOnum -->
  ...
  {"&quot;"     ,34}, // quotation mark = APL quote, U+0022 ISOnum -->
  {"&amp;"      ,38}, // ampersand, U+0026 ISOnum -->
  {"&lt;"       ,60}, // less-than sign, U+003C ISOnum -->
  {"&gt;"       ,62}, // greater-than sign, U+003E ISOnum -->
